...
interface WikivoyageQueryResponse {
  query?: {
    pages?: Record<string, WikivoyagePage>;
    geosearch?: Array<{ title: string; pageid: number }>;
  };
}
...
export class DiscoverService {
  ...
  async findNearbyPOIs(
    ...
    try {
      ...
      this.logger.debug(`Resolved Wikivoyage zone for ${lat},${lng}: ${city ?? 'null'}`);
      if (city) {
        ...
        try {
          ...
          this.logger.warn(`Smart discovery failed for zone ${city}, falling back: ${msg}`);
        }
      }
      ...
      throw new HttpException(`Failed to fetch nearby places: ${msg}`, HttpStatus.BAD_GATEWAY);
    }
  }
  ...
  private async resolveWikivoyageZone(lat: number, lng: number): Promise<string | null> {
    ...
    try {
      const { data } = await firstValueFrom(
        this.httpService.get<WikivoyageQueryResponse>(apiUrl, {
          headers,
          params: {
            action: 'query',
            list: 'geosearch',
      ...
      const msg = e instanceof Error ? e.message : String(e);
      this.logger.error(`Wikivoyage zone resolution error: ${msg}`);
      return null;
    }
  }
  ...
  async smartDiscovery(city: string, weightsStr?: string): Promise<ProcessedPOI[]> {
    try {
      ...
      return this.processSmartDiscovery(wikitext, title, cityLat || 0, cityLon || 0, weightsStr);
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
      }
      ...
      throw new HttpException(
        'Failed to perform smart discovery: ' + msg,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }

  private processSmartDiscovery(
    wikitext: string,
    title: string,
    cityLat: number,
    cityLon: number,
    ...
    const listings = this.parseWikivoyageListings(wikitext);
    const hasActiveWeights = Object.keys(weights).length > 0;
    const poiMap = new Map<string, WikivoyageListing>();
    ...
    for (const listing of uniqueListings) {
      ...
      if (hasActiveWeights) {
        for (const [category, weight] of Object.entries(weights)) {
          ...
          if (
            catLower === 'shopping' &&
            (typeLower === 'buy' || descLower.includes('shop') || descLower.includes('mall'))
          ) {
            match = true;
          }
          if (
          ...
          if (
            ...
            (typeLower === 'see' ||
              ...
              descLower.includes('art'))
          ) {
            match = true;
          }
          if (
            catLower === 'food' &&
            (typeLower === 'eat' || descLower.includes('restaurant') || descLower.includes('cafe'))
          ) {
            match = true;
          }
          if (
            ...
            (typeLower === 'drink' ||
              ...
              descLower.includes('club') ||
              descLower.includes('pub'))
          ) {
            match = true;
          }
          if (catLower === 'activities' && typeLower === 'do') {
            match = true;
          }
          if (
            catLower === 'monument' &&
      ...
      results.push({
        ...
        description: listing.content || listing.description || null,
      });
    }
    return results;
  }
  ...
  private async fetchCoordsFromOverpass(
    names: string[],
    lat: number,
    lng: number,
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      this.logger.warn(`Overpass geocoding failed: ${msg}`);
    }
    return coords;
  }
  ...
  private async fetchWikivoyageWikitext(
    title: string,
  ): Promise<{ wikitext: string; title: string; lat?: number; lon?: number } | null> {
    const apiUrl = 'https://en.wikivoyage.org/w/api.php';
    const headers = { 'User-Agent': 'TrippierBot/1.0' };
  ...
  private parseWikivoyageListings(wikitext: string): WikivoyageListing[] {
      ...
      if (nameMatch) {
        ...
        if (params['name']) {
          listings.push({
            ...
            lng:
              params['long'] || params['lon'] ? parseFloat(params['long'] || params['lon']) : null,
            phone: params['phone'],
